        ...
        let warehouseUsers = [];
        let quickBillingContext = null;
        let bulkInventoryPreviewRows = [];
        ...
        const ui = {
            ...
            bulkInventoryAccount: document.getElementById("bulkInventoryAccount"),
            bulkInventoryMode: document.getElementById("bulkInventoryMode"),
            bulkInventoryTrackingLevel: document.getElementById("bulkInventoryTrackingLevel"),
            bulkInventoryFileInput: document.getElementById("bulkInventoryFileInput"),
            bulkInventoryPaste: document.getElementById("bulkInventoryPaste"),
            bulkInventoryCreateSkus: document.getElementById("bulkInventoryCreateSkus"),
            bulkInventoryCreateLocations: document.getElementById("bulkInventoryCreateLocations"),
            bulkInventoryBillAdjustments: document.getElementById("bulkInventoryBillAdjustments"),
            bulkInventoryMeta: document.getElementById("bulkInventoryMeta"),
            bulkInventoryPreviewEmpty: document.getElementById("bulkInventoryPreviewEmpty"),
            bulkInventoryPreviewWrap: document.getElementById("bulkInventoryPreviewWrap"),
            bulkInventoryPreviewBody: document.getElementById("bulkInventoryPreviewBody"),
            bulkInventoryMessage: document.getElementById("bulkInventoryMessage"),
            ...
            appUserEmail: document.getElementById("appUserEmail"),
            appUserRole: document.getElementById("appUserRole"),
            warehouseUserMessage: document.getElementById("warehouseUserMessage"),
            warehouseUserName: document.getElementById("warehouseUserName"),
            warehouseUserEmail: document.getElementById("warehouseUserEmail"),
            warehouseUserPassword: document.getElementById("warehouseUserPassword"),
            warehouseUserRole: document.getElementById("warehouseUserRole"),
            warehouseUserActive: document.getElementById("warehouseUserActive"),
            warehouseUsersMeta: document.getElementById("warehouseUsersMeta"),
            warehouseUsersList: document.getElementById("warehouseUsersList"),
        ...
        function init() {
            ...
            document.getElementById("bulkInventoryForm")?.addEventListener("submit", saveBulkInventoryUpdate);
            document.getElementById("previewBulkInventoryBtn")?.addEventListener("click", previewBulkInventoryRows);
            document.getElementById("clearBulkInventoryBtn")?.addEventListener("click", clearBulkInventoryForm);
            document.getElementById("generateCycleCountWorksheetBtn")?.addEventListener("click", generateCycleCountWorksheet);
            document.getElementById("downloadBulkTemplateBtn")?.addEventListener("click", downloadBulkInventoryTemplate);
            ui.bulkInventoryFileInput?.addEventListener("change", handleBulkInventoryFileInput);
            [ui.adjustAccount, ui.adjustLocation, ui.transferAccount, ui.transferFrom, ui.transferTo, ui.moveAccount, ui.moveFrom, ui.moveTo].forEach((input) => {
            ...
            document.getElementById("clearReportFilterBtn").addEventListener("click", () => {
                ...
                ui.reportOwner.value = "";
                renderReports("");
                ui.reportOwner.focus();
            });
            ...
            ui.billingActivityMapBody?.addEventListener("input", onBillingActivityMapDraftChange);
            ui.billingActivityMapBody?.addEventListener("change", onBillingActivityMapDraftChange);
            ...
            document.getElementById("openQuickBillingBtn")?.addEventListener("click", openQuickBillingFromReceiving);
            document.getElementById("warehouseUserForm")?.addEventListener("submit", saveWarehouseUser);
            ui.warehouseUsersList?.addEventListener("click", onWarehouseUsersListClick);
            ...
            ui.scanAccount.value = state.preferences.lastAccount || "";
            ui.scanLocation.value = state.preferences.lastLocation || "";
            ui.masterItemAccount.value = state.preferences.lastAccount || "";
            ...
            ui.palletLabelAccount.value = labelToolState.palletAccount || state.preferences.lastAccount || "";
            ...
            ui.billingOwner.value = state.preferences.lastAccount || "";
            ...
            renderAll();
            setSection(isMobileView() ? "mobile-home" : "scan");
            syncServerState(false).catch(() => {});
            refreshPortalAccessList().catch(() => {});
            refreshPortalOrdersList().catch(() => {});
            refreshCurrentAppUser().catch(() => {});
            refreshWarehouseUsers(true).catch(() => {});
        ...
        function toggleDesktopPreviewMode() {
            ...
            setSection(activeSection === "mobile-home" ? "scan" : activeSection);
        }
        ...
        function setSection(section) {
            ...
            if (section === "billing" && !isMobileView()) ui.billingOwner.focus();
            syncServerState(true).catch(() => {});
        }
        ...
        function setDesktopSubview(group, view) {
            document.querySelectorAll(`[data-desktop-subview-group="${group}"]`).forEach((element) => {
                element.classList.toggle("active", element.dataset.desktopSubview === view);
            });
            document.querySelectorAll(`[data-desktop-subview-btn-group="${group}"]`).forEach((button) => {
                const isActive = button.dataset.desktopSubviewTarget === view;
        ...
        function setMobileSubview(group, view) {
            ...
            if (group === "actions") {
                activeActionSubview = view;
                setDesktopSubview(group, view);
            }
        ...
        function initMobileAutocomplete() {
            ...
            [ui.scanAccount, ui.searchAccount, ui.adjustAccount, ui.transferAccount, ui.moveAccount, ui.reportOwner, ui.masterOwnerName, ui.masterItemAccount, ui.palletLabelAccount]
                .forEach((input) => registerMobileAutocomplete(input, {
                    ...
                    onSelect: (option, currentInput) => {
                        if (currentInput === ui.scanAccount) {
                            state.preferences.lastAccount = option.value;
                            saveState();
            ...
            registerMobileAutocomplete(ui.palletLabelSku, {
                ...
                getEmptyState: () => norm(ui.palletLabelAccount.value)
                    ? "No SKUs are saved for this vendor / customer yet."
                    : "Choose vendor / customer first so the pallet SKU list stays owner-safe.",
                onSelect: () => {
                    syncPalletLabelCatalogFields();
                    return true;
                }
            ...
            registerMobileAutocomplete(ui.scanUpc, {
                ...
                getEmptyState: () => norm(ui.scanAccount.value)
                    ? "No UPCs are saved for this vendor / customer yet."
                    : "Choose vendor / customer first. Then the UPC wheel will only show that owner's items.",
                onSelect: (option) => {
            ...
            registerMobileAutocomplete(ui.scanSku, {
                ...
                getEmptyState: () => norm(ui.scanAccount.value)
                    ? "No SKUs are saved for this vendor / customer yet."
                    : "Choose vendor / customer first. Then the SKU wheel will only show that owner's items.",
                onSelect: (option) => {
                    applyMobileScanItem(option);
                    return false;
                }
        ...
        function getMobilePickerKicker(input) {
            if (input === ui.scanAccount || input === ui.searchAccount || input === ui.adjustAccount || input === ui.transferAccount || input === ui.moveAccount || input === ui.masterOwnerName || input === ui.masterItemAccount || input === ui.reportOwner) {
                return "Owner Picker";
            }
            if (input === ui.scanLocation || input === ui.adjustLocation || input === ui.transferFrom || input === ui.transferTo || input === ui.moveFrom || input === ui.moveTo || input === ui.masterLocationCode) {
        ...
        function addToBatch(event) {
            ...
            const accountName = norm(ui.scanAccount.value || state.preferences.lastAccount);
            ...
            if (!accountName) return fail(ui.scanMessage, "Please scan or enter a vendor / customer.", ui.scanAccount);
        ...
        async function performSingleSearch(rawValue, announce = true) {
            ...
            const ownerFilter = norm(ui.searchAccount.value);
            if (!query) return showMessage(ui.searchMessage, "Enter a SKU, UPC, or location to search.", "error");
            try {
                await syncServerState(true);

                const matches = filterInventoryByOwner(ownerFilter).filter((line) => searchMode === "sku"
        ...
        async function performMultiSearch(rawValue, announce = true) {
            ...
            try {
                ...
                const groups = terms.map((term) => {
                    const matches = filterInventoryByOwner(ownerFilter).filter((line) => line.sku.includes(term) || (line.upc || "").includes(term));
        ...
        function renderStats() {
            const trackedTotals = summarizeTrackedTotals(state.inventory);
            const locations = new Set(state.inventory.map((line) => line.location)).size;
            ui.statUnits.textContent = formatTrackedSummary(trackedTotals);
            ui.statLines.textContent = num(state.inventory.length);
        ...
        function renderDatalists() {
            ...
            ui.locationList.innerHTML = [...locationMap.values()]
                ...
                .join("");
            ui.ownerList.innerHTML = [...ownerMap.values()]
                .sort((a, b) => a.value.localeCompare(b.value))
                .map((entry) => `<option value="${attr(entry.value)}">${esc(entry.label)}</option>`)
                .join("");
        ...
        function renderActionSkuDatalists() {
            ...
            const adjustLocations = getOwnerScopedLocationOptions(ui.adjustAccount?.value || "", { mode: "source" });
            const transferFromLocations = getOwnerScopedLocationOptions(ui.transferAccount?.value || "", { mode: "source" });
            const transferToLocations = getOwnerScopedLocationOptions(ui.transferAccount?.value || "", {
                mode: "destination",
                excludeLocation: ui.transferFrom?.value || ""
            });
            const moveFromLocations = getOwnerScopedLocationOptions(ui.moveAccount?.value || "", { mode: "source" });
            const moveToLocations = getOwnerScopedLocationOptions(ui.moveAccount?.value || "", {
                mode: "destination",
                excludeLocation: ui.moveFrom?.value || ""
            });

            const adjustItems = getLocationInventoryItems(ui.adjustAccount?.value || "", ui.adjustLocation?.value || "");
            const transferItems = getLocationInventoryItems(ui.transferAccount?.value || "", ui.transferFrom?.value || "");
            ...
            const syncLocationField = (input, locations, waitingText, readyText) => {
                ...
                const scopeReady = input === ui.adjustLocation
                    ? !!norm(ui.adjustAccount?.value)
                    : input === ui.transferFrom
                        ? !!norm(ui.transferAccount?.value)
                        : input === ui.transferTo
                            ? !!norm(ui.transferAccount?.value) && !!norm(ui.transferFrom?.value)
                            : input === ui.moveFrom
                                ? !!norm(ui.moveAccount?.value)
                                : !!norm(ui.moveAccount?.value) && !!norm(ui.moveFrom?.value);
            ...
            const syncField = (input, items, waitingText, readyText) => {
                ...
                const scopeReady = input === ui.transferSku
                    ? !!norm(ui.transferAccount?.value) && !!norm(ui.transferFrom?.value)
                    : !!norm(ui.adjustAccount?.value) && !!norm(ui.adjustLocation?.value);
            ...
            syncLocationField(ui.adjustLocation, adjustLocations, "Choose vendor/customer first", "Select a location for this vendor/customer");
            syncLocationField(ui.transferFrom, transferFromLocations, "Choose vendor/customer first", "Select the source location");
            syncLocationField(ui.transferTo, transferToLocations, "Choose vendor/customer and source location first", "Select a valid destination location");
            syncLocationField(ui.moveFrom, moveFromLocations, "Choose vendor/customer first", "Select the source location");
            syncLocationField(ui.moveTo, moveToLocations, "Choose vendor/customer and source location first", "Select a valid destination location");
            syncField(ui.adjustSku, adjustItems, "Choose vendor/customer and location first", "Select a SKU from this location");
            syncField(ui.transferSku, transferItems, "Choose vendor/customer and source location first", "Select a SKU from the source location");
        }
        ...
        function syncScanItemSelectors() {
            const owner = norm(ui.scanAccount.value);
            ...
            ui.scanSku.placeholder = owner
                ? (ownerItems.length ? "Select a SKU for this vendor / customer" : "No saved SKUs for this vendor / customer")
                : "Choose vendor / customer first";
            ui.scanUpc.placeholder = owner
                ? (ownerItems.some((item) => !!item.upc) ? "Select a UPC for this vendor / customer" : "No saved UPCs for this vendor / customer")
                : "Choose vendor / customer first";
        ...
        function syncPalletLabelSkuOptions() {
            const accountName = norm(ui.palletLabelAccount.value || "");
            ...
            ui.palletLabelSku.placeholder = accountName
                ? (items.length ? "Choose a SKU for this vendor / customer" : "No saved SKUs for this vendor / customer")
                : "Choose vendor / customer first";
        ...
        async function savePalletLabelRecord(event, { printAfterSave = false } = {}) {
            ...
            if (!entry) {
                return fail(ui.labelsMessage, "Vendor / Customer, SKU, cases on pallet, and date are required for a pallet label.", ui.palletLabelAccount);
            }
        ...
        async function saveMasterOwner(event) {
            ...
            if (!entry?.name) return fail(ui.catalogMessage, "Enter a vendor / customer name to save.", ui.masterOwnerName);
            ...
            try {
                ...
                await syncServerState(true);
                state.preferences.lastAccount = entry.name;
                saveState();
                ...
                showMessage(ui.catalogMessage, `Saved vendor / customer ${entry.name}${entry.portalLoginEmail ? ` with linked portal email ${entry.portalLoginEmail}` : ""}.`, "success");
        ...
        function loadVendorToPortalAccess() {
            ...
            if (!entry?.name) return fail(ui.portalAccessMessage, "Enter or load a vendor / customer profile first.", ui.masterOwnerName);
            ui.portalAccessAccount.value = entry.name;
        ...
        function loadVendorToBilling() {
            ...
            if (!entry?.name) return fail(ui.billingMessage, "Enter or load a vendor / customer profile first.", ui.masterOwnerName);
        ...
        function onBillingOwnerInput() {
            const owner = norm(ui.billingOwner.value);
            if (owner) {
                state.preferences.lastAccount = owner;
                saveState();
            }
        ...
        function getFilteredBillingEvents() {
            const ownerFilter = norm(ui.billingOwner.value);
        ...
        function getBillingActivityCatalog() {
            const raw = Array.isArray(state.billing?.activityCatalog) && state.billing.activityCatalog.length
                ? state.billing.activityCatalog
                : BILLING_ACTIVITY_CATALOG;
            const unique = new Map();
            raw.map(sanitizeBillingActivityCatalog).filter(Boolean).forEach((entry) => unique.set(entry.code, entry));
            return [...unique.values()];
        }

        function getBillingActivityMapForOwner(owner) {
            const ownerName = norm(owner);
            if (!ownerName) return [];
            return (state.billing?.activityMap || []).filter((row) => row.accountName === ownerName);
        }

        function getBillingActivityMapDraftForOwner(owner) {
            const ownerName = norm(owner);
            if (!billingActivityMapDrafts.has(ownerName)) {
                const seeded = new Map();
                getBillingActivityMapForOwner(ownerName).forEach((entry) => seeded.set(entry.activityCode, { ...entry }));
                billingActivityMapDrafts.set(ownerName, seeded);
            }
            return billingActivityMapDrafts.get(ownerName);
        }

        function getEffectiveBillingActivityMapRows(owner) {
            const ownerName = norm(owner);
            const draft = ownerName ? getBillingActivityMapDraftForOwner(ownerName) : new Map();
            return getBillingActivityCatalog().map((activity) => {
                const override = draft.get(activity.code);
                return {
                    activityCode: activity.code,
                    activityName: activity.name,
                    activityDescription: activity.description,
                    feeCode: override?.feeCode || '',
                    autoCreate: override?.autoCreate === true,
                    defaultQuantity: toNumber(override?.defaultQuantity) || 1,
                    note: override?.note || ''
                };
            });
        }

        function renderBillingActivityMap(owner) {
            const ownerName = norm(owner);
            const rows = ownerName ? getEffectiveBillingActivityMapRows(ownerName) : [];
            ui.billingActivityMapMeta.textContent = ownerName ? `${num(rows.length)} activity${rows.length === 1 ? '' : 'ies'}` : 'Choose a vendor / customer';
            if (!ownerName) {
                ui.billingActivityMapEmpty.classList.remove('hidden');
                ui.billingActivityMapWrap.classList.add('hidden');
                ui.billingActivityMapBody.innerHTML = '';
                ui.billingActivityMapEmpty.textContent = 'Choose a vendor / customer to set up billing activity mapping.';
                return;
            }
            ui.billingActivityMapEmpty.classList.add('hidden');
            ui.billingActivityMapWrap.classList.remove('hidden');
            const feeRows = getEffectiveBillingRows(ownerName).filter((row) => row.isEnabled);
            const fallbackRows = feeRows.length ? feeRows : getBillingFeeCatalogRows();
            const feeOptions = [`<option value="">No default fee</option>`].concat(fallbackRows.map((fee) => `<option value="${attr(fee.code)}">${esc(`${fee.category} - ${fee.name}`)}</option>`)).join('');
            ui.billingActivityMapBody.innerHTML = rows.map((row) => `
                <tr data-billing-activity-code="${attr(row.activityCode)}">
                    <td><input type="checkbox" data-billing-activity-auto ${row.autoCreate ? 'checked' : ''}></td>
                    <td>
                        <div class="billing-fee-name">
                            <strong>${esc(row.activityName)}</strong>
                            <span class="billing-fee-code">${esc(row.activityCode)}</span>
                            <span class="meta">${esc(row.activityDescription || '')}</span>
                        </div>
                    </td>
                    <td><select data-billing-activity-fee>${feeOptions}</select></td>
                    <td><input type="number" min="0.01" step="0.01" data-billing-activity-qty value="${attr(String(toNumber(row.defaultQuantity) || 1))}"></td>
                    <td><input type="text" data-billing-activity-note value="${attr(row.note || '')}" placeholder="Optional note / rule"></td>
                </tr>
            `).join('');
            rows.forEach((row) => {
                const select = ui.billingActivityMapBody.querySelector(`tr[data-billing-activity-code="${cssEscape(row.activityCode)}"] [data-billing-activity-fee]`);
                if (select) select.value = row.feeCode || '';
            });
        }

        function onBillingActivityMapDraftChange(event) {
            const row = event.target.closest('[data-billing-activity-code]');
            if (!row) return;
            const owner = norm(ui.billingOwner.value);
            if (!owner) return;
            const activityCode = row.dataset.billingActivityCode || '';
            const draft = getBillingActivityMapDraftForOwner(owner);
            draft.set(activityCode, {
                accountName: owner,
                activityCode,
                feeCode: norm(row.querySelector('[data-billing-activity-fee]')?.value || ''),
                autoCreate: row.querySelector('[data-billing-activity-auto]')?.checked === true,
                defaultQuantity: toNumber(row.querySelector('[data-billing-activity-qty]')?.value) || 1,
                note: String(row.querySelector('[data-billing-activity-note]')?.value || '').trim()
            });
        }

        async function saveBillingActivityMap() {
            const owner = norm(ui.billingOwner.value);
            if (!owner) return fail(ui.billingMessage, 'Choose the vendor / customer first.', ui.billingOwner);
            const mappings = getEffectiveBillingActivityMapRows(owner).map((entry) => ({
                accountName: owner,
                activityCode: entry.activityCode,
                feeCode: entry.feeCode,
                autoCreate: entry.autoCreate === true,
                defaultQuantity: toNumber(entry.defaultQuantity) || 1,
                note: String(entry.note || '').trim()
            }));
            try {
                await requestJson('/api/billing/activity-map', {
                    method: 'POST',
                    body: JSON.stringify({ accountName: owner, mappings })
                });
                billingActivityMapDrafts.delete(owner);
                await syncServerState(true);
                showMessage(ui.billingMessage, `Saved billing activity map for ${owner}.`, 'success');
            } catch (error) {
                showMessage(ui.billingMessage, error.message, 'error');
            }
        }

        function getQuickBillingMappedFee(owner, activityCode) {
            const ownerName = norm(owner);
            const activity = norm(activityCode);
            if (!ownerName || !activity) return null;
            return getEffectiveBillingActivityMapRows(ownerName).find((row) => row.activityCode === activity && row.feeCode) || null;
        }
        ...
        function renderBilling() {
            ...
            ui.billingRatesMeta.textContent = owner ? `${num(feeRows.length)} fee${feeRows.length === 1 ? "" : "s"}` : "Choose a vendor / customer";

            renderBillingFeeOptions(owner);
            renderBillingActivityMap(owner);

            if (!owner) {
                ui.billingRatesEmpty.textContent = "Choose a vendor / customer to assign warehouse billing rates.";
                ui.billingRatesEmpty.classList.remove("hidden");
                ui.billingRatesWrap.classList.add("hidden");
                ui.billingRatesBody.innerHTML = "";
                ui.billingManualFeeMeta.textContent = "Choose a vendor / customer and fee to add a manual billing line.";
        ...
        function syncManualBillingFeeDefaults() {
            ...
            ui.billingManualFeeMeta.textContent = `${fee.name} | ${fee.unitLabel || "No unit label"} | ${fee.isEnabled ? "Enabled" : "Disabled for this owner"}`;
        }
        ...
        async function exportBillingDetailCsv() {
            try {
                await syncServerState(true);
                const rows = [["Service Date", "Owner", "Fee Code", "Fee Name", "Category", "Quantity", "Unit", "Rate", "Amount", "Status", "Invoice Number", "Reference", "Source Type", "Source Ref", "Note"]]
        ...
        async function exportBillingZohoCsv() {
            try {
                ...
                const rows = [["Customer Name", "Billing Email", "Invoice Number", "Invoice Date", "Service Date", "Item Name", "Description", "Quantity", "Rate", "Amount", "Reference", "Source", "Status"]]
                    .concat(getFilteredBillingEvents().map((event) => {
        ...
        async function savePortalAccess(event) {
            event.preventDefault();
            const accountName = norm(ui.portalAccessAccount.value);
            ...
            if (!accountName) {
                return fail(ui.portalAccessMessage, "Choose the vendor / customer for this portal login.", ui.portalAccessAccount);
            }
            if (!email) {
                return fail(ui.portalAccessMessage, "Enter the login email address for this customer portal user.", ui.portalAccessEmail);
            }
            if (!password && !portalAccessRecords.some((entry) => entry.accountName === accountName)) {
                return fail(ui.portalAccessMessage, "Set a password the first time you enable portal access for a vendor.", ui.portalAccessPassword);
            }

            try {
                await requestJson("/api/admin/portal-access", {
                    method: "POST",
                    body: JSON.stringify({ accountName, email, password, isActive })
                });
                state.preferences.lastAccount = accountName;
                saveState();
                ui.scanAccount.value = accountName;
                ...
                showMessage(ui.portalAccessMessage, `${isActive ? "Saved" : "Updated"} portal access for ${accountName} linked to ${email}${password ? " with a new password" : ""}.`, "success");
            } catch (error) {
        ...
        function renderPortalAccessList(errorText = "") {
            ui.portalAccessCount.textContent = `${num(portalAccessRecords.length)} saved`;

            if (!portalAccessRecords.length) {
                ui.portalAccessList.innerHTML = `<p class="empty">${esc(errorText || "No vendor portal logins saved yet.")}</p>`;
                return;
            }

            ui.portalAccessList.innerHTML = portalAccessRecords.map((entry) => `
                <div class="quick-row">
                    <div class="quick-main">
                        <strong>${esc(entry.accountName)}</strong>
                        <div class="quick-meta">
                            ${entry.email ? `Email ${esc(entry.email)} | ` : ""}${entry.isActive ? "Portal active" : "Portal disabled"}${entry.lastLoginAt ? ` | Last login ${esc(formatDate(entry.lastLoginAt))}` : " | No portal login yet"}
                        </div>
                    </div>
                    <button class="btn ghost mini" type="button" data-edit-portal-access="${attr(entry.accountName)}">Load</button>
                </div>
            `).join("");
        }
        ...
        function onPortalAccessListClick(event) {
            ...
            const accountName = editButton.dataset.editPortalAccess || "";
            ui.portalAccessAccount.value = accountName;
            const match = portalAccessRecords.find((entry) => entry.accountName === accountName);
        ...
        async function refreshPortalOrdersList(silent = false) {
            try {
                const payload = await requestJson("/api/admin/portal-orders");
                portalOrderRecords = Array.isArray(payload?.orders)
        ...
        function renderPortalOrdersList(errorText = "") {
            ui.portalOrdersMeta.textContent = `${num(portalOrderRecords.length)} order${portalOrderRecords.length === 1 ? "" : "s"}`;

            if (!portalOrderRecords.length) {
                ui.portalOrdersList.innerHTML = `<p class="empty">${esc(errorText || "No vendor portal orders yet.")}</p>`;
                return;
            }

            ui.portalOrdersList.innerHTML = portalOrderRecords.map((order) => {
                const lineSummary = order.lines.length
                    ? order.lines.slice(0, 3).map((line) => `${line.sku} (${formatTrackedQuantity(line.quantity, line.trackingLevel)})`).join(" | ")
                    : "No lines";
                const hiddenCount = Math.max(0, order.lines.length - 3);
                ...
                return `
                    <div class="quick-row">
                        <div class="quick-main">
                            ...
                            <div class="quick-meta">${esc(order.contactName || "No contact")} ${order.contactPhone ? `| ${esc(order.contactPhone)}` : ""}</div>
                            ...
                            <div class="quick-meta">${esc(lineSummary)}${hiddenCount ? ` | +${hiddenCount} more` : ""}</div>
                        ...
                        <div class="quick-row-actions">
                            ...
                            ${nextStatus ? `<button class="btn ghost mini" type="button" data-portal-order-status="${attr(order.id)}" data-next-status="${attr(nextStatus)}">${esc(portalOrderActionLabel(nextStatus))}</button>` : ""}
                            <button class="btn ghost mini" type="button" data-portal-order-billing="${attr(order.id)}">Add Fee Activity</button>
                        </div>
                    </div>
                `;
            }).join("");
        ...
        async function onPortalOrdersListClick(event) {
            const billingButton = event.target.closest("[data-portal-order-billing]");
            if (billingButton) {
                const orderId = billingButton.dataset.portalOrderBilling || "";
                const order = portalOrderRecords.find((entry) => String(entry.id) === String(orderId));
                if (order) {
                    openQuickBilling({
                        owner: order.accountName,
                        reference: order.poNumber || order.shippingReference || order.orderCode,
                        note: `Portal order ${order.orderCode}`
                    });
                }
                return;
            }
        ...
        function openQuickBillingFromReceiving() {
            openQuickBilling({
                owner: norm(ui.scanAccount?.value || state.preferences.lastAccount || ""),
                reference: `RCV-${todayInputValue()}`,
                note: 'Receiving activity',
                activityCode: 'RECEIVING_INBOUND'
            });
        }

        function openQuickBilling(context = {}) {
            quickBillingContext = context || {};
            setSection('billing');
            if (context.owner) {
                ui.billingOwner.value = context.owner;
                onBillingOwnerInput();
            }
            if (context.reference) ui.billingManualReference.value = context.reference;
            const mapped = getQuickBillingMappedFee(context.owner, context.activityCode);
            if (mapped?.feeCode) ui.billingManualFeeCode.value = mapped.feeCode;
            if (mapped?.defaultQuantity) ui.billingManualQuantity.value = String(mapped.defaultQuantity);
            if (mapped?.note || context.note) ui.billingManualNote.value = [mapped?.note, context.note].filter(Boolean).join(' | ');
            syncManualBillingFeeDefaults();
            ui.billingManualQuantity.focus();
        }
        ...
        async function saveMasterItem(event) {
            event.preventDefault();
            const accountName = norm(ui.masterItemAccount.value);
            ...
            if (!accountName) return fail(ui.catalogMessage, "Enter the vendor / customer before saving an item master.", ui.masterItemAccount);
        ...
        function beginMasterItemEdit(item) {
            ...
            ui.masterItemEditorMeta.textContent = "SKU changes update matching inventory lines for this vendor / customer. Vendor / customer stays locked during edit.";
            refreshImagePreview({
                urlInput: ui.masterItemImageUrl,
                previewWrap: ui.masterItemImagePreviewWrap,
                previewImg: ui.masterItemImagePreview,
        ...
        function cancelMasterItemEdit() {
            resetMasterItemForm(state.preferences.lastAccount || "");
        ...
        function renderMasterLibrary() {
            ...
            const ownerMatches = [...state.masters.ownerRecords]
                .filter((entry) => !query || entry.name.includes(query) || norm(entry.note).includes(query))
        ...
        function renderMasterOwnerRows(owners, totalMatches, query) {
            if (!owners.length) {
                return `<p class="empty">${query ? `No saved vendors / customers matched "${esc(query)}".` : "No vendors / customers saved yet. Add them here so they show up in autocomplete."}</p>`;
            }
            ...
            const rows = owners.map((entry) => {
                ...
                <div class="quick-row">
                    <div class="quick-main">
                        ...
                        <div class="quick-meta">${esc(meta || entry.note || "Saved vendor / customer")}</div>
            ...
            if (owners.length < totalMatches) {
                return `${rows}<p class="meta">Showing the first ${num(owners.length)} of ${num(totalMatches)} vendors / customers. Narrow the filter to see more.</p>`;
            }
        ...
        function fillScanFromOwner(name) {
            ...
            showMessage(ui.catalogMessage, `Loaded vendor / customer ${normalized} into the scan form.`, "info");
        }
        ...
        function fillScanFromMasterItem(code, options = {}) {
            ...
            state.preferences.lastAccount = item.accountName;
            saveState();
            syncScanItemSelectors();
        ...
        function syncScanFieldsFromCatalog() {
            syncScanItemSelectors();
            const accountName = norm(ui.scanAccount.value);
            const sku = norm(ui.scanSku.value);
            const upc = norm(ui.scanUpc.value);
        ...
        function renderReports(filterText) {
            const ownerFilter = norm(ui.reportOwner.value);
        ...
        async function exportOwnerReportCsv() {
            try {
                await syncServerState(true);
                const rows = [["Owner", "Locations", "PalletLocations", "TotalPallets", "TotalCases", "TotalUnits"]].concat(
                    getOwnerReportRows(ui.reportFilter.value, norm(ui.reportOwner.value)).map((row) => [
                        row.owner,
                ...
                showMessage(ui.reportMessage, "Owner utilization report exported as CSV.", "success");
            } catch (error) {
        ...
        async function exportLocationReportCsv() {
            try {
                await syncServerState(true);
                const rows = [["Owner", "Location", "Note", "Lines", "Pallets", "UniqueSkus", "TotalCases", "TotalUnits"]].concat(
        ...
        async function exportItemReportCsv() {
            try {
                await syncServerState(true);
                const rows = [["Owner", "SKU", "UPC", "Description", "Tracking", "Locations", "Lines", "TotalQty"]].concat(
        ...
        async function exportVendorInventoryReportCsv() {
            try {
                await syncServerState(true);
                const rows = [["Owner", "Location", "SKU", "UPC", "Description", "Tracking", "Qty", "Updated"]].concat(
                    getVendorInventoryReportRows(ui.reportFilter.value, norm(ui.reportOwner.value)).map((row) => [
                        row.owner,
                        row.location,
                        row.sku,
                        row.upc || "",
                ...
                showMessage(ui.reportMessage, "Vendor inventory report exported as CSV.", "success");
            } catch (error) {
                showMessage(ui.reportMessage, error.message, "error");
            }
        }
        ...
        function printReports() {
            ...
            openPrintWindow(
                ...
                `
                    ...
                    <p>Owner Filter: ${esc(ownerFilter || "ALL")}</p>
                    <p>Filter: ${esc(norm(ui.reportFilter.value) || "ALL")}</p>
                    <section>
                        <h2>Owner Utilization</h2>
                        ${reportTableMarkup(
                            ["Owner", "Locations", "Pallet Locations", "Total Pallets", "Units / Cases"],
                    ...
                    <section>
                        ...
                        ${reportTableMarkup(
                            ["Owner", "Location", "Note", "Lines", "Pallets", "Unique SKUs", "Units / Cases"],
                    ...
                    <section>
                        <h2>Item Report</h2>
                        ${reportTableMarkup(
                            ["Owner", "SKU", "UPC", "Description", "Tracking", "Locations", "Lines", "Total Qty"],
                    ...
                    <section>
                        <h2>Inventory By Vendor</h2>
                        ${reportTableMarkup(
                            ["Owner", "Location", "SKU", "UPC", "Description", "Tracking", "Qty", "Updated"],
        ...
        function getVendorInventoryReportRows(filterText = "", ownerFilter = "") {
            ...
            return filterInventoryByOwner(ownerFilter)
        ...
        function renderSingleSearch(query, mode, matches) {
            ...
            desktopMarkup = `
                <div class="summary-grid">
                    ...
                    <div class="summary"><span>Owners / SKUs</span><strong>${num(owners)} / ${num(skus)}</strong></div>
                </div>
        ...
        function renderActivity() {
            ...
            ui.activityList.innerHTML = state.activity.slice(0, 12).map((item) => `
        ...
        function renderInventory(filterText) {
            const query = norm(filterText || "");
            const matches = sortedInventory().filter((line) => inventoryMatchesQuery(line, query));
        ...
        function renderSummary() {
            const locations = new Set(state.inventory.map((line) => line.location)).size;
            const owners = getOwnerOptions().length;
            const ownerSkus = new Set(state.inventory.map((line) => `${line.accountName}::${line.sku}`)).size;
            const trackedTotals = summarizeTrackedTotals(state.inventory);
        ...
        async function removeQuantity() {
            const accountName = norm(ui.adjustAccount.value);
            ...
            if (!accountName || !location || !skuOrUpc || !quantity) return showMessage(ui.actionMessage, "Enter vendor/customer, location, SKU or UPC, and a quantity to remove.", "error");
            ...
            if (!matchingLine) {
                return showMessage(ui.actionMessage, "That SKU or UPC is not in the selected vendor/customer and location.", "error");
            }
        ...
        async function deleteLine() {
            ...
            const location = norm(ui.adjustLocation.value);
            const skuOrUpc = norm(ui.adjustSku.value);
            if (!accountName || !location || !skuOrUpc) return showMessage(ui.actionMessage, "Enter vendor/customer, location, and SKU or UPC to delete the line.", "error");
            const matchingLine = findClientInventoryLine(accountName, location, skuOrUpc);
            if (matchingLine?.duplicateUpc) {
                return showMessage(ui.actionMessage, "Multiple items matched that UPC in the selected location. Use the SKU instead.", "error");
            }
        ...
        async function transferQuantity(event) {
            event.preventDefault();
            const accountName = norm(ui.transferAccount.value);
            ...
            if (!accountName || !from || !to || !skuOrUpc || !quantity) return showMessage(ui.actionMessage, "Fill in vendor/customer, from location, to location, SKU or UPC, and quantity.", "error");
            ...
            if (!matchingLine) {
                return showMessage(ui.actionMessage, "That SKU or UPC is not in the selected source location for this vendor/customer.", "error");
            }
            if (quantity > Number(matchingLine.quantity || 0)) {
            ...
            if (conflictingOwners.length) {
                return showMessage(ui.actionMessage, `Cannot transfer into ${to} because it already contains another vendor/customer: ${conflictingOwners.join(", ")}.`, "error");
            }
        ...
        async function moveAllItems(event) {
            event.preventDefault();
            const accountName = norm(ui.moveAccount.value);
            const from = norm(ui.moveFrom.value);
            const to = norm(ui.moveTo.value);
            if (!accountName || !from || !to) return showMessage(ui.actionMessage, "Enter vendor/customer plus both the source and destination locations.", "error");
            if (from === to) return showMessage(ui.actionMessage, "Source and destination locations cannot be the same.", "error");
            const sourceLines = state.inventory.filter((line) => norm(line.accountName) === accountName && norm(line.location) === from);
            if (!sourceLines.length) {
                return showMessage(ui.actionMessage, "The selected source location does not have inventory for that vendor/customer.", "error");
            }
            const conflictingOwners = getForeignOwnersAtLocation(accountName, to);
            if (conflictingOwners.length) {
                return showMessage(ui.actionMessage, `Cannot move into ${to} because it already contains another vendor/customer: ${conflictingOwners.join(", ")}.`, "error");
            }
        ...
        function parseBulkInventoryRows(rawText) {
            const text = String(rawText || "").replace(/\r/g, "").trim();
            if (!text) return [];
            const lines = text.split("\n").map((line) => line.trim()).filter(Boolean);
            if (!lines.length) return [];
            const header = parseDelimitedRow(lines[0]).map((cell) => cell.toLowerCase().replace(/[^a-z0-9]/g, ""));
            const hasHeader = header.includes("sku") || header.includes("location") || header.includes("qty") || header.includes("quantity");
            const rows = [];
            ...
            const getValue = (cells, names, fallbackIndex) => {
                for (const name of names) {
                    const idx = header.indexOf(name);
                    if (idx >= 0) return cells[idx] || "";
                }
                return fallbackIndex != null ? (cells[fallbackIndex] || "") : "";
            };
            for (let i = startIndex; i < lines.length; i += 1) {
                const cells = parseDelimitedRow(lines[i]);
                const row = {
                    sku: norm(getValue(cells, ["sku", "item", "itemcode"], 0)),
                    location: norm(getValue(cells, ["location", "bin", "bincode"], 1)),
                    qty: Number.parseFloat(getValue(cells, ["qty", "quantity", "count"], 2) || "0"),
                    lpn: norm(getValue(cells, ["lpn", "pallet", "licenseplate"], 3)),
                    reason: getValue(cells, ["reason", "note", "notes"], 4).trim(),
                    description: getValue(cells, ["description", "desc", "itemdescription"], 5).trim(),
                    upc: norm(getValue(cells, ["upc", "barcode"], 6)),
                    trackingLevel: norm(getValue(cells, ["trackinglevel", "tracking", "uom"], 7)).toUpperCase()
                };
                if (!row.sku && !row.location && !row.qty && !row.reason && !row.description && !row.upc) continue;
                rows.push(row);
            }
            return rows.filter((row) => row.sku && row.location && Number.isFinite(row.qty));
        }

        function renderBulkInventoryPreview(rows) {
            const list = Array.isArray(rows) ? rows : [];
            ui.bulkInventoryMeta.textContent = `${num(list.length)} parsed row${list.length === 1 ? "" : "s"} ready.`;
            if (!list.length) {
                ui.bulkInventoryPreviewBody.innerHTML = "";
                ui.bulkInventoryPreviewWrap.classList.add("hidden");
                ui.bulkInventoryPreviewEmpty.classList.remove("hidden");
                return;
            }
            ui.bulkInventoryPreviewBody.innerHTML = list.map((row) => `
                <tr>
                    <td>${esc(row.sku)}</td>
                    <td>${esc(row.location)}</td>
                    ...
                    <td>${esc(row.description || "")}</td>
                    <td>${esc(row.upc || "")}</td>
                    <td>${esc((row.trackingLevel || "") || (ui.bulkInventoryTrackingLevel.value || ""))}</td>
                </tr>
            `).join("");
            ui.bulkInventoryPreviewWrap.classList.remove("hidden");
            ui.bulkInventoryPreviewEmpty.classList.add("hidden");
        }

        function getBulkInventoryParsedRows() {
            const rows = parseBulkInventoryRows(ui.bulkInventoryPaste.value);
            const trackingFallback = norm(ui.bulkInventoryTrackingLevel.value).toUpperCase();
            return rows.map((row) => ({ ...row, trackingLevel: row.trackingLevel || trackingFallback || "" }));
        }

        function previewBulkInventoryRows() {
            bulkInventoryPreviewRows = getBulkInventoryParsedRows();
            renderBulkInventoryPreview(bulkInventoryPreviewRows);
            if (!bulkInventoryPreviewRows.length) {
                showMessage(ui.bulkInventoryMessage, "No valid rows were found. Include at least SKU, Location, and Qty columns.", "error");
                return;
            }
            showMessage(ui.bulkInventoryMessage, `Parsed ${bulkInventoryPreviewRows.length} row${bulkInventoryPreviewRows.length === 1 ? "" : "s"}.`, "success");
        }

        function clearBulkInventoryForm() {
            document.getElementById("bulkInventoryForm").reset();
            bulkInventoryPreviewRows = [];
            renderBulkInventoryPreview([]);
            showMessage(ui.bulkInventoryMessage, "Bulk inventory form cleared.", "success");
        }

        function handleBulkInventoryFileInput(event) {
            const file = event.target.files && event.target.files[0];
            if (!file) return;
            const reader = new FileReader();
            reader.onload = () => {
                ui.bulkInventoryPaste.value = String(reader.result || "");
                previewBulkInventoryRows();
            };
            reader.readAsText(file);
        }

        function downloadBulkInventoryTemplate() {
            const rows = [
                ["sku", "location", "qty", "lpn", "reason", "description", "upc", "trackingLevel"],
                ["SKU-1001", "A-01-01", "25", "STO-0001", "opening balance", "Sample item", "", "CASE"],
                ["SKU-1002", "A-01-02", "-5", "", "damage", "", "", ""]
            ];
            downloadBlob(rows.map((row) => row.map(csvCell).join(",")).join("\r\n"), `wms365-bulk-inventory-template-${fileStamp()}.csv`, "text/csv;charset=utf-8");
            showMessage(ui.bulkInventoryMessage, "Bulk inventory template downloaded.", "success");
        }

        function generateCycleCountWorksheet() {
            const owner = norm(ui.bulkInventoryAccount.value || ui.reportOwner?.value || "");
            const lines = sortedInventory().filter((line) => !owner || line.accountName === owner);
            if (!lines.length) {
                showMessage(ui.bulkInventoryMessage, "No inventory lines were found for the selected vendor/customer.", "error");
                return;
            }
            const rows = [["owner", "location", "sku", "upc", "description", "tracking", "systemQty", "countedQty", "variance", "reason"]].concat(lines.map((line) => [
                line.accountName,
                line.location,
                line.sku,
                line.upc || "",
            ...
            downloadBlob(rows.map((row) => row.map(csvCell).join(",")).join("\r\n"), `wms365-cycle-count-${owner || "all"}-${fileStamp()}.csv`, "text/csv;charset=utf-8");
            showMessage(ui.bulkInventoryMessage, `Cycle count worksheet downloaded for ${owner || "all owners"}.`, "success");
        }

        async function saveBulkInventoryUpdate(event) {
            event.preventDefault();
            const accountName = norm(ui.bulkInventoryAccount.value);
            const rows = getBulkInventoryParsedRows();
            if (!accountName) {
                showMessage(ui.bulkInventoryMessage, "Vendor / customer is required.", "error");
                return;
            }
            if (!rows.length) {
                showMessage(ui.bulkInventoryMessage, "Add at least one valid bulk inventory row before saving.", "error");
                return;
            }
            try {
                const result = await requestJson("/api/inventory/bulk-update", {
                    method: "POST",
                    body: JSON.stringify({
                        accountName,
                        mode: ui.bulkInventoryMode.value,
                        createMissingSkus: ui.bulkInventoryCreateSkus.checked,
                        createMissingLocations: ui.bulkInventoryCreateLocations.checked,
                        triggerBillingSuggestion: ui.bulkInventoryBillAdjustments.checked,
                        defaultTrackingLevel: ui.bulkInventoryTrackingLevel.value,
                        rows
                    })
                });
                bulkInventoryPreviewRows = rows;
                renderBulkInventoryPreview(rows);
                await syncServerState(true);
                if (ui.bulkInventoryBillAdjustments.checked) {
                    const qtyTotal = rows.reduce((sum, row) => sum + Math.abs(Number(row.qty) || 0), 0) || rows.length;
                    openQuickBilling({
                        owner: accountName,
                        activityCode: "REWORK_SPECIAL_PROJECT",
                        reference: `Bulk inventory update ${todayInputValue()}`,
                        quantity: qtyTotal,
                        note: `Bulk inventory update (${result.mode || ui.bulkInventoryMode.value}) for ${rows.length} row(s).`
                    });
                }
                showMessage(ui.bulkInventoryMessage, result.message || `Bulk inventory update saved for ${rows.length} row(s).`, "success");
            } catch (error) {
                showMessage(ui.bulkInventoryMessage, error.message, "error");
            }
        }
        ...
        function printSingleSearch() {
            ...
            openPrintWindow(
                ...
                `
                    ...
                    <p>Owner Filter: ${esc(lastSingleSearch.ownerFilter || "ALL")}</p>
        ...
        function printMultiSearch() {
            ...
            openPrintWindow(
                ...
                `
                    ...
                    <p>Owner Filter: ${esc(lastMultiSearch.ownerFilter || "ALL")}</p>
        ...
        async function exportCsv() {
            try {
                await syncServerState(true);
                const rows = [["Owner", "Location", "SKU", "UPC", "Description", "Tracking", "Quantity", "CreatedAt", "UpdatedAt"]].concat(
                    sortedInventory().map((line) => [
                        line.accountName,
                        line.location,
                        line.sku,
                        line.upc || "",
        ...
        function applyServerState(payload) {
            ...
            state.billing.activityMap = Array.isArray(payload?.billing?.activityMap) ? payload.billing.activityMap.map(sanitizeBillingActivityMap).filter(Boolean) : [];
            state.billing.activityCatalog = Array.isArray(payload?.billing?.activityCatalog) ? payload.billing.activityCatalog.map(sanitizeBillingActivityCatalog).filter(Boolean) : BILLING_ACTIVITY_CATALOG;
        ...
        async function refreshCurrentAppUser() {
            try {
                const payload = await requestJson('/api/app/me');
                currentAppUser = sanitizeAppUser(payload?.user);
            } catch (_error) {
                currentAppUser = null;
            }
        ...
        async function refreshWarehouseUsers(silent = false) {
            try {
                const payload = await requestJson('/api/app/users');
                warehouseUsers = Array.isArray(payload?.users) ? payload.users.map(sanitizeAppUser).filter(Boolean) : [];
                renderWarehouseUsers();
            } catch (error) {
                if (!silent) showMessage(ui.warehouseUserMessage, error.message, 'error');
                warehouseUsers = [];
                renderWarehouseUsers(error.message);
            }
        }

        function renderWarehouseUsers(errorText = '') {
            if (ui.appUserName) ui.appUserName.textContent = currentAppUser?.fullName || 'Not loaded';
            if (ui.appUserEmail) ui.appUserEmail.textContent = currentAppUser?.email || '—';
            if (ui.appUserRole) ui.appUserRole.textContent = currentAppUser?.role || '—';
            if (!ui.warehouseUsersList || !ui.warehouseUsersMeta) return;
            ui.warehouseUsersMeta.textContent = `${num(warehouseUsers.length)} user${warehouseUsers.length === 1 ? '' : 's'}`;
            if (!warehouseUsers.length) {
                ui.warehouseUsersList.innerHTML = `<p class="empty">${esc(errorText || 'No warehouse users loaded.')}</p>`;
                return;
            }
            ui.warehouseUsersList.innerHTML = warehouseUsers.map((user) => `
                <div class="quick-row">
                    <div class="quick-main">
                        <strong>${esc(user.fullName || user.email)}</strong>
                        <div class="quick-meta">${esc(user.email)} | ${esc(user.role)} | ${user.isActive ? 'Active' : 'Disabled'}</div>
                        <div class="quick-meta">${user.lastLoginAt ? `Last login ${esc(formatDate(user.lastLoginAt))}` : 'No login recorded yet.'}</div>
                    </div>
                    <div class="quick-row-actions">
                        <span class="pill ${user.isActive ? 'success' : 'warn'}">${user.isActive ? 'ACTIVE' : 'DISABLED'}</span>
                        <button class="btn ghost mini" type="button" data-user-toggle="${attr(user.id)}" data-user-active="${attr(String(!user.isActive))}">${user.isActive ? 'Disable' : 'Enable'}</button>
                        <button class="btn ghost mini" type="button" data-user-reset="${attr(user.id)}">Reset Password</button>
                    </div>
                </div>
            `).join('');
        }

        async function saveWarehouseUser(event) {
            event.preventDefault();
            const fullName = String(ui.warehouseUserName.value || '').trim();
            const email = norm(ui.warehouseUserEmail.value);
            const password = String(ui.warehouseUserPassword.value || '');
            const role = norm(ui.warehouseUserRole.value || 'warehouse').toLowerCase();
            const isActive = ui.warehouseUserActive.value !== 'false';
            if (!fullName) return fail(ui.warehouseUserMessage, 'Enter the warehouse user name.', ui.warehouseUserName);
            if (!email) return fail(ui.warehouseUserMessage, 'Enter the warehouse user email.', ui.warehouseUserEmail);
            if (password.length < 8) return fail(ui.warehouseUserMessage, 'Password must be at least 8 characters.', ui.warehouseUserPassword);
            try {
                await requestJson('/api/app/users', {
                    method: 'POST',
                    body: JSON.stringify({ fullName, email, password, role, isActive })
                });
                event.target.reset();
                ui.warehouseUserRole.value = 'warehouse';
                ui.warehouseUserActive.value = 'true';
                showMessage(ui.warehouseUserMessage, `Saved warehouse user ${fullName}.`, 'success');
                await refreshWarehouseUsers(true);
            } catch (error) {
                showMessage(ui.warehouseUserMessage, error.message, 'error');
            }
        }

        async function onWarehouseUsersListClick(event) {
            const toggleButton = event.target.closest('[data-user-toggle]');
            if (toggleButton) {
                try {
                    await requestJson(`/api/app/users/${toggleButton.dataset.userToggle}/status`, {
                        method: 'POST',
                        body: JSON.stringify({ isActive: toggleButton.dataset.userActive === 'true' })
                    });
                    await refreshWarehouseUsers(true);
                    showMessage(ui.warehouseUserMessage, 'Warehouse user status updated.', 'success');
                } catch (error) {
                    showMessage(ui.warehouseUserMessage, error.message, 'error');
                }
                return;
            ...
            const nextPassword = window.prompt('Enter the new password for this warehouse user (minimum 8 characters).');
            if (!nextPassword) return;
            try {
                await requestJson(`/api/app/users/${resetButton.dataset.userReset}/reset-password`, {
                    method: 'POST',
                    body: JSON.stringify({ password: nextPassword })
                });
                showMessage(ui.warehouseUserMessage, 'Warehouse password reset complete.', 'success');
            } catch (error) {
                showMessage(ui.warehouseUserMessage, error.message, 'error');
            }
        }
        ...
        function defaultState() {
            return {
                ...
                billing: { feeCatalog: [], ownerRates: [], activityMap: [], activityCatalog: BILLING_ACTIVITY_CATALOG, events: [] },
                preferences: { lastLocation: "", lastAccount: "", desktopMobilePreview: false },
        ...
        function sanitizeLocalCache(raw) {
            return {
                ...
                billing: {
                    ...
                    activityMap: Array.isArray(raw?.billing?.activityMap) ? raw.billing.activityMap.map(sanitizeBillingActivityMap).filter(Boolean) : [],
                    activityCatalog: Array.isArray(raw?.billing?.activityCatalog) ? raw.billing.activityCatalog.map(sanitizeBillingActivityCatalog).filter(Boolean) : BILLING_ACTIVITY_CATALOG,
        ...
        function mapItemCsvRows(rows) {
            ...
            return bodyRows.map((cells, index) => {
                ...
                if (!normalizedAccountName || !normalizedSku) {
                    throw new Error(`Row ${rowNumber} is missing Vendor / Customer or SKU.`);
                }

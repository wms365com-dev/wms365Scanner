        ...
        const DESKTOP_SECTION_META = {
            scan: {
                title: "Receiving",
                meta: "Stage inbound inventory, confirm location and quantity, then save the open batch to live stock."
            },
            search: {
                title: "Lookup",
                meta: "Search by SKU, UPC, or BIN and review live inventory in the active company."
            },
            actions: {
                title: "Adjustments",
                meta: "Remove, transfer, and move stock with company-safe location controls."
            },
            inventory: {
                title: "Inventory Control",
                meta: "Review the live warehouse table and operational inventory summary."
            },
            reports: {
                title: "Reports",
                meta: "Run company utilization, location, item, and inventory worksheet exports."
            },
            billing: {
                title: "Billing",
            ...
            backup: {
                title: "Data Admin",
                meta: "Export, import, and safeguard shared warehouse data."
            }
        };
        ...
        function init() {
            ...
            ui.activeCompany.value = state.preferences.activeCompany || state.preferences.lastAccount || "";
            ui.scanAccount.value = state.preferences.activeCompany || state.preferences.lastAccount || "";
            ui.scanLocation.value = state.preferences.lastLocation || "";
            ui.masterItemAccount.value = state.preferences.activeCompany || state.preferences.lastAccount || "";
            ...
            ui.palletLabelAccount.value = labelToolState.palletAccount || state.preferences.activeCompany || state.preferences.lastAccount || "";
            ...
            ui.billingOwner.value = state.preferences.activeCompany || state.preferences.lastAccount || "";
        ...
        function getActiveCompany() {
            return norm(ui.activeCompany?.value || state.preferences.activeCompany || state.preferences.lastAccount || "");
        }
        ...
        function updateWorkspaceCommandBar() {
            ...
            if (ui.workspaceCompanyChip) {
                ui.workspaceCompanyChip.textContent = company ? `Company: ${company}` : "All Companies";
            }
        ...
        function renderPortalOrdersList(errorText = "") {
            const company = getActiveCompany();
            const visibleOrders = company
                ? portalOrderRecords.filter((order) => order.accountName === company)
                : portalOrderRecords;
            ui.portalOrdersMeta.textContent = `${num(visibleOrders.length)} order${visibleOrders.length === 1 ? "" : "s"}`;

            if (!visibleOrders.length) {
                ui.portalOrdersList.innerHTML = `<p class="empty">${esc(errorText || "No company portal orders yet.")}</p>`;
                return;
            }
                    ...
                    <div class="quick-row">
                        ...
                        ${(order.status === "STAGED" || order.status === "SHIPPED") ? `
                            <div class="portal-order-shipping-box">
                                ...
                                <div class="quick-meta">Upload POD, signed BOL, packing slip, or carrier confirmation. Images are compressed before upload.</div>
                                ...
                                <div class="quick-row-actions">
                                    <button class="btn ${order.status === "STAGED" ? "" : "secondary"}" type="button" data-portal-ship-confirmation="${attr(order.id)}" data-portal-transition="${attr(order.status === "STAGED" ? "SHIPPED" : "SHIPPED")}">
                                        ${esc(order.status === "STAGED" ? "Mark Shipped + Save Docs" : "Update Shipping Confirmation")}
                                    </button>
                                </div>
                            </div>
                        ` : ""}
        ...
        function formatPortalOrderShipTo(order) {
            return [order.shipToName, order.shipToAddress1, [order.shipToCity, order.shipToState, order.shipToPostalCode].filter(Boolean).join(", "), order.shipToCountry || ""]
                .filter(Boolean)
                .join("<br>");
        }
        ...
        function buildPackingSlipHtml(order) {
            const shipFrom = ["Grey Wolf 3PL & Logistics Inc.", "1330 Courtney Park Dr E", "Mississauga, ON L5T 1K5"].join("<br>");
            const lineRows = order.lines.map((line) => `
                <tr>
                    <td>${esc(line.sku)}</td>
                    <td>${esc(line.description || "")}</td>

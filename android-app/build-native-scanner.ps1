...
if (!(Test-Path $jdkHome)) { throw "Missing local JDK at $jdkHome." }
if (!(Test-Path $androidSdk)) { throw "Missing local Android SDK at $androidSdk." }
if (!(Test-Path $gradle)) { throw "Missing local Gradle at $gradle." }
...
try {
    & $gradle :nativeScanner:assembleDebug "-PWMS365_BASE_URL=$baseUrl" --no-daemon
} finally {
    Pop-Location
}
